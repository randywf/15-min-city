<script lang="ts">
  ...
  let open = false; // falls du das schon hast: so lassen

  // wenn Info-Modal aufgeht: LayerPanel sicher schließen
  $: if ($infoOpen) open = false;
</script>
...
{#if !$infoOpen}
  {#if hasData}
    ...
    >
      <div
        class="bg-white/95 backdrop-blur border border-gray-200 shadow-lg rounded-lg overflow-hidden"
      >
        <button
        ...
        {#if panelExpanded}
          <div class="border-t max-h-96 overflow-y-auto">
              ...
              <div class="p-3 border-b bg-gray-50">
                <label class="flex items-center justify-between cursor-pointer">
                  ...
                  <input
                    ...
                    on:change={(e) =>
                      onToggleIsochrone(e.currentTarget.checked)}
                  />
                </label>
              </div>
            {/if}
            ...
            {#if poiData?.amenities && poiData.amenities.length > 0}
              <div class="p-2">
                <div class="text-xs font-semibold text-gray-600 px-2 py-1.5">
                  POI Categories
                </div>
